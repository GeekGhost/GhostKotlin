...
import cn.com.ghostkotlin.callback.VideoCallBack
...
class VideoDetailActivity : BaseActivity(), VideoDetailContract.View {
    ...
    private val mAdapter by lazy { VideoDetailAdapter(this, itemList,R.layout.item_video_detail_info) }
     ...
     * Item 详细数据
     */
    private lateinit var itemData: VideoDetailBean
    private var orientationUtils: OrientationUtils? = null

    private var itemList = ArrayList<VideoDetailBean.Ret.X.Child>()
...
//                .into(imageView)
        ...
        mVideoView.setStandardVideoAllCallBack(object : VideoCallBack {
...
//        itemData = intent.getSerializableExtra(Constants.BUNDLE_VIDEO_DATA) as VideoDetailBean
...
//        saveWatchVideoHistoryInfo(itemData)
     ...
     * 保存观看记录
     */
    private fun saveWatchVideoHistoryInfo(watchItem: VideoDetailBean) {
        //保存之前要先查询sp中是否有该value的记录，有则删除.这样保证搜索历史记录不会有重复条目
        val historyMap = WatchHistoryUtils.getAll(Constants.FILE_HISTORY_NAME, MyApplication.context) as Map<*, *>
        for ((key, _) in historyMap) {
            if (watchItem == WatchHistoryUtils.getObject(Constants.FILE_HISTORY_NAME, MyApplication.context, key as String)) {
                WatchHistoryUtils.remove(Constants.FILE_HISTORY_NAME, MyApplication.context, key)
            }
        }
        WatchHistoryUtils.putObject(Constants.FILE_HISTORY_NAME, MyApplication.context, watchItem, "" + mFormat.format(Date()))
    }
     ...
     * 设置视频信息
     */
    override fun setVideoInfo(itemInfo: VideoDetailBean) {
        itemData = itemInfo
...
//        mPresenter.requestRelatedVideo(itemInfo.data?.id?:0)
     ...
     * 设置相关的数据视频
     */
    override fun setRecentRelatedVideo(itemList: ArrayList<VideoDetailBean.Ret.X.Child>) {
